...
<template>
  <header class="glass-topbar flex items-center justify-between px-6 flex-shrink-0">
    ...
    <!-- 右侧搜索框 -->
    <div class="relative">
      <div class="search-box flex items-center gap-2 px-3.5 py-2 rounded-xl bg-black/[0.06] backdrop-blur-sm border border-white/50 shadow-sm transition-all duration-200">
...
.glass-topbar {
  height: 70px;
  background: rgba(var(--glass-r), var(--glass-g), var(--glass-b), 0.60);
  backdrop-filter: blur(40px) saturate(180%);
  -webkit-backdrop-filter: blur(40px) saturate(180%);
  border-bottom: 0.5px solid rgba(var(--glass-r), var(--glass-g), var(--glass-b), 0.4);
...
.search-box {
  ...
  border: 0.5px solid rgba(var(--glass-r), var(--glass-g), var(--glass-b), 0.4);
  box-shadow: inset 0 1px 0 rgba(255, 255, 255, 0.5);
  transition: all 0.25s cubic-bezier(0.4, 0, 0.2, 1);
}
.search-box:focus-within {
  border-color: rgba(79, 140, 255, 0.3);
  background: rgba(var(--glass-r), var(--glass-g), var(--glass-b), 0.65);
  box-shadow:
